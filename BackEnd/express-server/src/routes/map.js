...
// [API] FastAPI 재난 활성 목록 프록시
router.get('/disaster/active', async (req, res) => {
  try {
    const response = await axios.get(`${process.env.FASTAPI_URL}/disaster/active`);
    res.json(response.data);
  } catch (error) {
    console.error('[WARN] disaster/active failed:', error.message);
